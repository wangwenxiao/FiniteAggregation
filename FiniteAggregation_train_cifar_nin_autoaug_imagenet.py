...
checkpoint_subdir = f'./{checkpoint_dir}/' + dirbase + f'_FiniteAggregation_k{args.k}_d{args.d}_autoaug_imagenet'
...
for part in range(args.start, args.start + args.range):
    ...
    print('here')
    trainloader = torch.utils.data.DataLoader(torch.utils.data.Subset(trainset,part_indices), batch_size=16, shuffle=True, num_workers=1)
